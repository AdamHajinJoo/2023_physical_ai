...
def lines_filtered(d_line_arr, d_slope_degree):
    ...
    L_mean_line = np.expand_dims(np.mean(L_lines, axis=0), axis=0).astype(int)
    R_mean_line = np.expand_dims(np.mean(R_lines, axis=0), axis=0).astype(int)
...
def main():
    ...
    _, _, all_lines, _, _, mean_line = lines_filtered(line_arr, slope_degree)
